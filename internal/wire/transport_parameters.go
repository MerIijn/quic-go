...
type TransportParameters struct {
	...
	// EmitChromeQUIC makes Marshal (client) emit the Chrome-150 transport
	// parameter set/values/GREASE instead of quic-go's defaults.
	EmitChromeQUIC bool
}
...
func (p *TransportParameters) Marshal(pers protocol.Perspective) []byte {
	if p.EmitChromeQUIC && pers == protocol.PerspectiveClient {
...
// marshalChrome emits the QUIC transport parameters exactly as Chrome 150 does:
// the same set, values, GREASE, version_information and google_connection_options,
// and notably omitting max_ack_delay / active_connection_id_limit / ack_delay_exponent.
...
func (p *TransportParameters) marshalChrome() []byte {
	...
	rand.Read(greaseVer)
	greaseVer[0] = greaseVer[0]&0xf0 | 0x0a
	greaseVer[1] = greaseVer[0]
