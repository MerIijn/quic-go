...
// quicConn is the subset of the crypto/tls QUICConn API used by the handshake.
// Both tls.QUICConn (default) and tls.UQUICConn (uTLS fingerprinted) satisfy it,
// letting us pick the ClientHello fingerprint at connection setup.
...
func NewCryptoSetupClient(
	...
	case clientHelloSpec != nil:
		...
		// in the preset path, so swap the placeholder for a GenericExtension that
		// carries our exact quic-go-marshaled transport parameters (Chrome set,
		// with the real source connection ID).
		tpBytes := cs.ourParams.Marshal(protocol.PerspectiveClient)
		spec := *clientHelloSpec
		exts := make([]tls.TLSExtension, len(clientHelloSpec.Extensions))
		copy(exts, clientHelloSpec.Extensions)
		for i, ext := range exts {
			if _, ok := ext.(*tls.QUICTransportParametersExtension); ok {
				exts[i] = &tls.GenericExtension{Id: 0x39, Data: tpBytes} // quic_transport_parameters (57)
			}
		}
...
func (h *cryptoSetup) handleEvent(ev tls.QUICEvent) (err error) {
	...
	case tls.QUICStoreSession:
		...
		if sc, ok := h.conn.(interface {
			...
			return sc.StoreSession(ev.SessionState)
		}
		return nil
