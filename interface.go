...
type Config struct {
	...
	// ClientHelloID selects the uTLS ClientHello fingerprint used for the QUIC
	// TLS handshake (client only). When nil, the default (non-fingerprinted)
	// uTLS ClientHello is used. Set it to e.g. a Chrome fingerprint to mimic a
	// real browser's QUIC ClientHello.
	ClientHelloID *tls.ClientHelloID

	// ClientHelloSpec, when set, is applied via UQUICConn.ApplyPreset to fully
	// control the QUIC ClientHello (must include a QUICTransportParametersExtension).
	// Takes precedence over ClientHelloID. This is how VibeTLS supplies its own
	// Chrome-150 QUIC ClientHello.
